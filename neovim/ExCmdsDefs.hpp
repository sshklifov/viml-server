...
struct exarg {
  ...
  std::size_t *arglens;         ///< length of command arguments
  std::size_t argc;             ///< number of command arguments
...
struct expand {
  ...
  std::size_t xp_pattern_len;        // bytes in xp_pattern before cursor
  char *xp_arg;                 // completion function
  LuaRef xp_luaref;             // Ref to Lua completion function
  int xp_script_ctx;            // SCTX for completion function (TODO)
...
typedef struct {
  ...
  int cmod_filter_regmatch;  ///< set by :filter /pat/ (TODO)
